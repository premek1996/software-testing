...
public class PaymentService {
    ...
    public void chargeCard(MakePaymentDTO dto) {
        Customer customer = customerRepository.findById(dto.getCustomerId())
                .orElseThrow(() -> new BadRequestException(ExceptionMessage.CUSTOMER_DOES_NOT_EXIST_WITH_GIVEN_ID));

        if (!isCurrencyAccepted(dto.getCurrency())) {
            throw new BadRequestException(ExceptionMessage.CURRENCY_IS_NOT_ACCEPTED);
        }
        ...
        if (!cardPaymentCharge.isCardDebited()) {
            throw new BadRequestException(ExceptionMessage.CARD_IS_NOT_DEBITED);
        }
